...
|---|---|
| **論理名** | 注文ヘッダー |
| **物理名** | `ec-data-platform-2026.02_preprocess.prep_orders` |
...
```
prep_orders.customer_id  → prep_customers.customer_id  （多:1）
prep_orders.order_id     ← prep_order_items.order_id   （1:多）
prep_orders.order_id     ← prep_order_reviews.order_id （1:多）
prep_orders.order_id     ← prep_order_payments.order_id（1:多）
```
