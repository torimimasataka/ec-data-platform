# テーブル定義書：prep_geolocation

| 項目 | 値 |
|---|---|
| **論理名** | 位置情報（郵便番号×緯度経度） |
| **物理名** | `ec-data-platform-2026.02_preprocess.prep_geolocation` |
...
```
prep_geolocation.geolocation_zip_code_prefix ← prep_customers.customer_zip_code_prefix（多:多）
prep_geolocation.geolocation_zip_code_prefix ← prep_sellers.seller_zip_code_prefix    （多:多）
※ 結合前に郵便番号単位で集約すること
```
