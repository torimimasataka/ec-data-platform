# テーブル定義書：prep_order_items

| 項目 | 値 |
|---|---|
| **論理名** | 注文明細 |
| **物理名** | `ec-data-platform-2026.02_preprocess.prep_order_items` |
...
```
prep_order_items.order_id   → prep_orders.order_id    （多:1）
prep_order_items.product_id → prep_products.product_id（多:1）
prep_order_items.seller_id  → prep_sellers.seller_id  （多:1）
```
